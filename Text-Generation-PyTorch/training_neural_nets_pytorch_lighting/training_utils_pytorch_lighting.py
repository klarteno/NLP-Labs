...
class TrainingPytorchLighting:
    def __init__(self, neural_net_model, text_data_module, optimizer_parameters):
    ...
    def train_net_model(self, max_epochs):
        ...
        self.text_generative_model = TextGenerativeNetModelPLighting(
            model_name=self.neural_net_model.to(self.DEVICE),
            optimizer_hparams=self.optimizer_parameters,
        )

        self.trainer_network_model.fit(
            model=self.text_generative_model, datamodule=self.text_data_module
        )
    ...
    def test_net_model(self):
        test_result = self.trainer_network_model.test(
            model=self.text_generative_model,
    ...
    def get_text_generative_model(self):
        return self.text_generative_model
